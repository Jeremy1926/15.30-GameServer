...
void Building::ServerCreateBuildingActor(UObject* Context, FFrame& Stack)
{
	...
	char _Unk_OutVar1;
	static auto CantBuild = (__int64 (*)(UWorld*, UObject*, FVector, FRotator, bool, TArray<ABuildingSMActor*> *, char*))(ImageBase + 0x278d0d0);
	...
	ABuildingSMActor* Building = Utils::SpawnActorUnfinished<ABuildingSMActor>(BuildingClass, CreateBuildingData.BuildLoc, CreateBuildingData.BuildRot, PlayerController);
	...
	Utils::FinishSpawnActor(Building, CreateBuildingData.BuildLoc, CreateBuildingData.BuildRot);
...
void Building::ServerEditBuildingActor(UObject* Context, FFrame& Stack)
{
	...
	static auto ReplaceBuildingActor = (ABuildingSMActor * (*)(ABuildingSMActor*, unsigned int, UObject*, unsigned int, int, bool, AFortPlayerController*))(ImageBase + 0x2429a70);
...
static inline std::unordered_map<std::string, uint32> AccoladeMapping;
void Building::OnDamageServer(ABuildingSMActor* Actor, float Damage, FGameplayTagContainer DamageTags, FVector Momentum, FHitResult HitInfo, AFortPlayerControllerAthena* InstigatedBy, AActor* DamageCauser, FGameplayEffectContextHandle EffectContext) {
	...
	if (ResCount > 0)
	{
		static auto AccoladeDef = Utils::FindObject<UFortAccoladeItemDefinition>(L"/Game/Athena/Items/Accolades/AccoladeId_066_WeakSpotsInARow.AccoladeId_066_WeakSpotsInARow");
		std::string PlayerName = InstigatedBy->PlayerState->GetPlayerName().ToString().c_str();
		uint32& AccoladeCount = AccoladeMapping[PlayerName];

		if (Damage == 100.f && AccoladeCount == 5)
		{
			AccoladeCount = 0;
			if (GameState->GamePhase != EAthenaGamePhase::Warmup)
			{
				GiveAccolade(InstigatedBy, AccoladeDef, nullptr, EXPEventPriorityType::NearReticle);
				AccoladeCount++;
			}
		}
		else
...
void Building::Hook() {
	...
	Utils::Hook(ImageBase + 0x3049130, OnDamageServer, OnDamageServerOG);
