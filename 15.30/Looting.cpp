...
bool Looting::ServerOnAttemptInteract(ABuildingContainer* BuildingContainer, AFortPlayerPawn*)
{
	...
	auto LootTierGroup = BuildingContainer->SearchLootTierGroup;
...
void Looting::SpawnConsumableActor(ABGAConsumableSpawner* Spawner)
{
	...
	FString ok(FName(L"FindGroundLocationAt"));

	auto GroundLoc = UFortKismetLibrary::FindGroundLocationAt(UWorld::GetWorld(), nullptr, Spawner->K2_GetActorLocation(), -1000, 2500, ok); // move to ground
...
void ServerAttemptInteract(UObject* Context, FFrame& Stack)
{
	...
	auto PC = ((AFortPlayerControllerAthena*)InteractionComp->GetOwner());

	if (!PC)
		return;

	callOG(InteractionComp, Stack.CurrentNativeFunction, ServerAttemptInteract, ReceivingActor, InteractComponent, InteractType, OptionalObjectData, InteractionBeingAttempted, RequestId);
	...
	{
		SendStatEvent(((AFortPlayerControllerAthena*)InteractionComp->GetOwner())->GetQuestManager(ESubGame::Athena), ReceivingActor, /*((AFortPlayerControllerAthena*)Comp->GetOwner())->MyFortPawn->GameplayTags*/Empty, TargetTags, &bruh, &bruh, 1, EFortQuestObjectiveStatEvent::Interact);
	}
}

void Looting::Hook()
{
	Utils::Hook(ImageBase + 0x023D4F80, SpawnLootHook);
	//Utils::Hook(ImageBase + 0x24282E0, PostUpdate, PostUpdateOG);
