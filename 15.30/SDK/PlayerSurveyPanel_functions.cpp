#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: PlayerSurveyPanel

#include "Basic.hpp"

#include "PlayerSurveyPanel_classes.hpp"
#include "PlayerSurveyPanel_parameters.hpp"


namespace SDK
{

// Function PlayerSurveyPanel.PlayerSurveyPanel_C.ExecuteUbergraph_PlayerSurveyPanel
// (Final, UbergraphFunction)
// Parameters:
// int32                                   EntryPoint                                             (BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)

void UPlayerSurveyPanel_C::ExecuteUbergraph_PlayerSurveyPanel(int32 EntryPoint)
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("ExecuteUbergraph_PlayerSurveyPanel");

	Params::PlayerSurveyPanel_C_ExecuteUbergraph_PlayerSurveyPanel Parms{};

	Parms.EntryPoint = EntryPoint;

	UObject::ProcessEvent(Func, &Parms);
}


// Function PlayerSurveyPanel.PlayerSurveyPanel_C.OnPlayerSurveyEnded
// (Event, Protected, BlueprintEvent)

void UPlayerSurveyPanel_C::OnPlayerSurveyEnded()
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("OnPlayerSurveyEnded");

	UObject::ProcessEvent(Func, nullptr);
}


// Function PlayerSurveyPanel.PlayerSurveyPanel_C.StartIntro_BP
// (Event, Public, BlueprintEvent)

void UPlayerSurveyPanel_C::StartIntro_BP()
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("StartIntro_BP");

	UObject::ProcessEvent(Func, nullptr);
}


// Function PlayerSurveyPanel.PlayerSurveyPanel_C.StartIntro
// (BlueprintCallable, BlueprintEvent)

void UPlayerSurveyPanel_C::StartIntro()
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("StartIntro");

	UObject::ProcessEvent(Func, nullptr);
}


// Function PlayerSurveyPanel.PlayerSurveyPanel_C.OutroDone
// (BlueprintCallable, BlueprintEvent)

void UPlayerSurveyPanel_C::OutroDone()
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("OutroDone");

	UObject::ProcessEvent(Func, nullptr);
}


// Function PlayerSurveyPanel.PlayerSurveyPanel_C.Construct
// (BlueprintCosmetic, Event, Public, BlueprintEvent)

void UPlayerSurveyPanel_C::Construct()
{
	static class UFunction* Func = nullptr;
	
	if (Func == nullptr)
		Func = Class->FindFunction("Construct");

	UObject::ProcessEvent(Func, nullptr);
}

}

